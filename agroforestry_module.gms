...
E_PLANTING_AF(hhold,field,inten,age_tree,y)$(ord(y) > 1 and p_field_AF(hhold,field) > 0)..
...
* Young trees production
            sum(NameYoung,p_Yld_C_tree(field, c_tree, NameYoung)) * pressuretree(hhold,c_tree,field,inten)*
            sum(age_tree$(ord(age_tree) < harvestingAge_tree(c_tree)), 
                V_Area_AF(hhold, field, c_tree,age_tree, inten, y)) +
* Adult trees production
            sum(NameAdult,p_Yld_C_tree(field, c_tree, NameAdult)) * pressuretree(hhold,c_tree,field,inten)*
            sum(age_tree$(ord(age_tree) >= harvestingAge_tree(c_tree) and ord(age_tree) < oldAge_tree(c_tree)), 
                V_Area_AF(hhold, field, c_tree,age_tree, inten, y)) +
* Old trees production
            sum(NameOld,p_Yld_C_tree(field, c_tree, NameOld)) * pressuretree(hhold,c_tree,field,inten) * 
...
$ifi %VALUECHAIN%==OFF   + sum((field, c_tree, inten),        sum(age_tree,(V0_Area_AF(hhold,field,c_tree,age_tree,inten)$(ord(y) = 1)*(1-p_buffer$(fieldcrop_tree(hhold,field))))) *        p_costPhyto_AF(hhold,c_tree,field,inten))
